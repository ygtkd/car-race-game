...
namespace CoastRacer {
 ...
 public sealed class BlenderWheels:MonoBehaviour {
  sealed class Wheel {public Transform rotor,steer;public float radius,spin;}
  ...
  public void Tick(float steering,float speed,float dt){
   if(!initialized){
    initialized=true;foreach(var t in GetComponentsInChildren<Transform>()){
     ...
     wheels.Add(new Wheel{rotor=t,steer=t.name.StartsWith("wheel_front")?mount:null,radius=Mathf.Max(.25f,mount.localPosition.y-.03f)});
    }
   }
