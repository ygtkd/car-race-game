...
{
 Properties { _Color("Color",Color)=(1,1,1,1) _Metallic("Metallic",Range(0,1))=0 _Smoothness("Smoothness",Range(0,1))=.2 _Grain("Grain",Range(0,1))=0 _Emission("Lamp",Range(0,1))=0 }
 SubShader {
 ...
 float _CoastNight,_Emission;float4 _CoastHeads[8],_CoastDirections[8];int _CoastLightCount;
 ...
 float3 n=normalize(i.normal),v=normalize(_WorldSpaceCameraPos.xyz-i.world),l=normalize(float3(-.4,.8,.3));
 ...
 float noise=frac(sin(dot(floor(i.world.xz*28),float2(12.9898,78.233)))*43758.5453);
 float3 baseColor=_Color.rgb*(1+(noise-.5)*_Grain);
