...
let a,b,c;
try{
 a=await connect();a.send('create',{track:'ridge',name:'通信甲',vehicle:'swift',badge:'winner~right~0.27~0.81~1.5~45~blue~hexagon~bolt'});const ja=await a.wait(x=>x.type==='joined');
 ...
 let state=await a.wait(x=>x.type==='state'&&x.cars.find(p=>p.id===ja.id)?.speed>4);let me=state.cars.find(p=>p.id===ja.id);
 assert(Math.abs(me.x)<50&&me.lap===0,'Client cannot spoof position or lap');
 assert(me.vehicle==='swift'&&me.badge==='winner~right~0.27~0.81~1.5~45~blue~hexagon~bolt'&&me.coins===0&&me.gauge<1,'Server owns vehicle, coins and gauge');
 ...
 fs.writeFileSync('Logs/revision6-protocol-checks.json',JSON.stringify({checks},null,2));
