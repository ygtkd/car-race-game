...
let first;try{
 ...
 await first.call('Page.navigate',{url:base+'/play/'});assert(await waitFor(first,'window.Racer&&window.testUnity&&document.querySelector("#loading").hidden',150),'Revision 9 WebGL loads');
 assert(await first.evaluate('!document.querySelector("#menuSettings")'), 'Menu text settings removed');
 ...
 await first.evaluate('document.querySelector("#settingsButton").click()');assert(await waitFor(first,'!document.querySelector("#settingsOverlay").hidden'),'Gear opens settings');
 await first.evaluate('document.querySelector("#closeSettings").click()');
 const awardCheck=await first.evaluate(`(()=>{const before=Club.profile.records.seen.length;Club.record([{id:'view',vehicle:'apex',finished:true,finishTime:100,rank:1,distance:100000,coins:300,specialUses:100,disruptions:50,peakSpeed:65}],{online:false,track:'shonan',raceId:'award-test'});Club.showAwards(()=>{});return [...document.querySelectorAll('#awardList .awardCopy')].map(e=>({title:e.querySelector('strong').textContent,reward:e.querySelector('small').textContent}))})()`);
 assert(awardCheck.length>5&&awardCheck.every(x=>x.title.endsWith('\u3092\u9054\u6210\uff01')&&x.reward.endsWith('\u3092\u30b2\u30c3\u30c8')&&!x.title.includes('\u9054\u6210\u3092\u9054\u6210')),'Paired achievement and reward cards');await screenshot(first,'revision9-awards.png');
 await first.evaluate('document.querySelector("#awardClose").click()');assert(await first.evaluate('!document.querySelector("#menu").inert'),'Award close restores input');
 ...
 await first.evaluate('document.querySelector("#solo").click()');assert(await waitFor(first,'Racer.telemetry.phase==="countdown"',120),'Countdown entered');await screenshot(first,'revision9-signals.png');assert(await first.evaluate('document.querySelector("#startLights").getBoundingClientRect().height<60'),'Compact signal height');
 ...
 for(const track of tracks.filter(t=>!process.argv.includes('--coast-only')||t.id==='shonan'))for(const night of [false,true]){
  ...
  for(const index of indices){await fixture(track,index,{},night);await screenshot(first,`revision9-view-${track.id}-${index}-${night?'night':'day'}.png`);}
 }
 ...
 await delay(2300);await screenshot(first,'revision9-finish-camera.png');assert(await first.evaluate('Racer.telemetry.cars[0].finished&&Racer.telemetry.cars[0].finishTime===100'),'Finish remains immutable during camera orbit');
 ...
 for(const mode of ['self-ring','remote-ring','remote-block']){
  ...
  await first.evaluate(`testUnity.SendMessage('RaceGame','NetworkSnapshot',JSON.stringify(${JSON.stringify({track:'shonan',phase:'race',raceId:mode,self:'view',cars:mode==='self-ring'?[me]:[me,other],coins:[],night:false,time:30})}))`);assert(await waitFor(first,`Racer.telemetry.raceId==='${mode}'`),'Guard visual fixture '+mode);await delay(600);await screenshot(first,'revision9-'+mode+'.png');
 }
 for(const vehicle of ['apex','swift','vortex','atlas','kebab','banana','tuktuk','bicycle','stormbike','aerobike']){
  await first.evaluate(`testUnity.SendMessage('RaceGame','CommandFromWeb',JSON.stringify({action:'showcar',vehicle:'${vehicle}',badge:''}));testUnity.SendMessage('RaceGame','CommandFromWeb',JSON.stringify({action:'badgeView',id:'front'}))`);assert(await waitFor(first,`Racer.telemetry.showroom&&Racer.telemetry.cars[0].vehicle==='${vehicle}'`),'Front model '+vehicle);await delay(600);await screenshot(first,'revision9-front-'+vehicle+'.png');
  await first.evaluate("testUnity.SendMessage('RaceGame','CommandFromWeb',JSON.stringify({action:'badgeView',id:'left'}))");await delay(600);await screenshot(first,'revision9-side-'+vehicle+'.png');
 }
 for(const vehicle of ['apex','swift','vortex','atlas','kebab'])for(const steer of [-1,1]){
  await fixture(track,240,{vehicle,steering:steer,speed:12},false);await first.evaluate("testUnity.SendMessage('RaceGame','CommandFromWeb',JSON.stringify({action:'rearView',rearView:true}))");await delay(700);await screenshot(first,`revision9-wheel-${vehicle}-${steer}.png`);
 }
 await first.evaluate("testUnity.SendMessage('RaceGame','CommandFromWeb',JSON.stringify({action:'rearView',rearView:false}))");
 ...
 assert(errors.length===0,'No revision 9 runtime errors');fs.writeFileSync(process.argv.includes('--coast-only')?'Logs/revision9-optimized-browser-checks.json':'Logs/revision9-browser-checks.json',JSON.stringify({checks,errors,note:'Desktop Edge touch emulation; scene snapshots used only for visual inspection.'},null,2));
