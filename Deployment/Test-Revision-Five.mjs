...
let first;try{
 ...
 await first.call('Page.navigate',{url:base+'/play/'});assert(await waitFor(first,'window.Racer && window.testUnity && document.querySelector("#loading").hidden'),'Revision 5 game loads');
 ...
 await screenshot(first,'revision5-road-trees.png');

 assert(await first.evaluate("getComputedStyle(document.querySelector('.nameplate')).backgroundColor==='rgba(0, 0, 0, 0)'"),'Nameplate background is transparent');
 assert(await first.evaluate("(()=>{const b=document.querySelector('#rearView').getBoundingClientRect(),a=document.querySelector('#throttle').getBoundingClientRect();return b.bottom<a.top&&b.width<=a.width})()"),'Rear button sits above accelerator');
 await screenshot(first,'revision5-finish-front.png');
 ...
 await first.call('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{id:1,...pedal}]});
 const rear=await first.evaluate("(()=>{const r=document.querySelector('#rearView').getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2}})()");
 await first.call('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{id:1,...pedal},{id:2,...rear}]});
 await first.call('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[{id:2,...rear}]});
 assert(await waitFor(first,'Racer.telemetry.rearView===true',5),'Rear camera activates with a second finger');
 assert(await first.evaluate('Racer.drive.throttle===1'),'Rear toggle retains held throttle');await screenshot(first,'revision5-rear-camera.png');
 await first.call('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});
 await first.evaluate("document.querySelector('#rearView').click()");assert(await waitFor(first,'Racer.telemetry.rearView===false',5),'Second tap restores front camera');
 await first.evaluate("document.querySelector('#rearView').click();document.querySelector('#pauseButton').click()");assert(await waitFor(first,'Racer.telemetry.rearView===false&&Racer.screen==="pause"',5),'Pause clears rear camera');
 await first.evaluate("document.querySelector('#resume').click()");await waitFor(first,'Racer.telemetry.phase==="race"');

 // Visual-only fixtures hold the late-effect snapshot during capture. Duration is tested by CoreTests and the real-driving test, not these screenshots.
 for(const vehicle of ['apex','swift','vortex','atlas','kebab','banana','tuktuk','bicycle']){
  ...
  await first.evaluate('fxFixture.cars[0].specialTime=5;fxFixture.cars[0].specialUses=1;testUnity.SendMessage("RaceGame","NetworkSnapshot",JSON.stringify(fxFixture))');assert(await waitFor(first,'Racer.telemetry.cars[0].specialUses===1 && Racer.telemetry.cars[0].specialTime>0',5),'Special snapshot applies '+vehicle);await screenshot(first,'revision5-special-'+vehicle+'.png');
  await first.evaluate('fxFixture.cars[0].specialTime=.75;testUnity.SendMessage("RaceGame","NetworkSnapshot",JSON.stringify(fxFixture));window.holdFX=setInterval(()=>testUnity.SendMessage("RaceGame","NetworkSnapshot",JSON.stringify(fxFixture)),100)');await delay(180);assert(await first.evaluate('Racer.telemetry.cars[0].specialTime>0 && Racer.telemetry.cars[0].specialTime<1'),'Special stays active near end '+vehicle);await screenshot(first,'revision5-special-late-'+vehicle+'.png');
  assert(await first.evaluate('document.querySelector("#specialButton").classList.contains("effectActive")'),'Special HUD active '+vehicle);await first.evaluate('clearInterval(window.holdFX)');
 }
 ...
 assert(errors.length===0,'No browser runtime errors');fs.writeFileSync('Logs/revision5-special-checks.json',JSON.stringify({checks,errors,note:'Desktop Edge touch emulation. Special screenshots use snapshot fixtures.'},null,2));
