...
var bad=BadgeDesign.Parse(Vehicles.Badge("finish~oops~NaN~Infinity~999~-999~hack~hack~hack"));Check(bad.x==.5f&&bad.y==.72f&&bad.size==1.25f&&bad.angle==-180&&bad.face=="top","Invalid badge settings normalized");
...
for(int race=0;race<10;race++){
 var track=new Track(race%2==0?"ridge":"suzuka");var session=new RaceSession(track);var cars=Enumerable.Range(0,8).Select(i=>{var c=Simulation.Spawn(track,i);c.id="test-"+i;c.vehicle=Vehicles.All[(i+race)%Vehicles.All.Length].id;return c;}).ToArray();var recoveries=new int[8];var grass=new float[8];
...
Check(uses.Count==Vehicles.All.Length,"All vehicle specials exercised in ten races");Directory.CreateDirectory("Logs");File.WriteAllText("Logs/revision6-ten-races.json",JsonSerializer.Serialize(new{races=rows,acceleration=new{normal=baseCar.speed,boosted=before},note="Ten automated races in the shared production driving engine. Not physical-device testing."},new JsonSerializerOptions{WriteIndented=true}));
