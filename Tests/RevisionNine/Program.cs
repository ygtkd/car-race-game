...
if(args.Contains("--boundaries")){Boundaries.Run();RevisionNineChecks.Run();return;}
if(args.Contains("--search")){
 ...
 File.WriteAllText("Logs/revision9-cpu-search.json",JsonSerializer.Serialize(scores,new JsonSerializerOptions{WriteIndented=true}));return;
}
...
var coast=new Track("shonan");int entrance=coast.Nearest(new Point(9.5f,7,-90));
Check(coast.distance[entrance]>=550&&coast.distance[entrance]<=650,"Start 600m before bridge");
Check(coast.points.Max(p=>p.y)>45,"Island road climbs above 45m");
Console.WriteLine("BRIDGE_DISTANCE "+coast.distance[entrance]);
...
File.WriteAllText("Logs/revision9-cpu.json",JsonSerializer.Serialize(compare,new JsonSerializerOptions{WriteIndented=true}));
...
File.WriteAllText("Logs/revision9-races.json",JsonSerializer.Serialize(reports,new JsonSerializerOptions{WriteIndented=true}));
