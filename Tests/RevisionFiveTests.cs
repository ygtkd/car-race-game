...
static class RevisionFiveTests {
 ...
 public static void Run(){
  ...
  session.Activate(pulse,new[]{pulse,target});Check(Math.Abs(target.speed-19.2f)<.001f&&target.jamTime==5,"pulse doubles range and reduction, lasts five seconds");
  foreach(var vehicle in Vehicles.All.Where(v=>new[]{"boost","dash","cadence","feast","grip","surf"}.Contains(v.special))){var normal=Simulation.Spawn(track);var powered=Simulation.Spawn(track);normal.vehicle=powered.vehicle=vehicle.id;powered.specialTime=5;Simulation.StepCar(normal,new DriveInput{throttle=1,assist=0},track,.02f);Simulation.StepCar(powered,new DriveInput{throttle=1,assist=0},track,.02f);float bonus=vehicle.special=="boost"||vehicle.special=="dash"?.70f:vehicle.special=="cadence"?.60f:vehicle.special=="feast"?.40f:.24f;Check(Math.Abs(powered.speed-normal.speed-vehicle.acceleration*bonus*.02f)<.0001f,vehicle.id+" doubled acceleration increment");}
 }
}
